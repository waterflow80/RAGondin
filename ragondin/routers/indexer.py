...
@router.post("/add-files/", response_model=None)
async def add_files(files: List[UploadFile] = File(...), milvus_crud: MilvusCRUD = Depends(get_milvus_crud)):
    try:
        # Create a temporary directory to store files
        temp_dir = Path("data/tmp")
        temp_dir.mkdir(parents=True, exist_ok=True)
        ...
        # Now pass the directory path to the Indexer
        await milvus_crud.add_files(path=temp_dir)
...
async def search(query_params: SearchRequest, milvus_crud: MilvusCRUD = Depends(get_milvus_crud)):
    try:
        ...
        # Perform the search using the Indexer
        results = await milvus_crud.search(query, top_k)
...
async def delete_files(request: DeleteFilesRequest, milvus_crud: MilvusCRUD = Depends(get_milvus_crud)):
    ...
    try:
        deleted_files, not_found_files = milvus_crud.delete_files(request.file_names)
        return {
            "message": "File processing completed.",
            "files_deleted": deleted_files,
            "files_not_found": not_found_files,
